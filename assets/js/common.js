...
if (patient && patientName && phoneNumberInput && noRekMedInput) {
  patient.addEventListener("change", () => {
    ...
    let splitValue = selectedPatient.split(" "); //return array
    function checkPhoneNumber(PhoneNumber) {
      if (PhoneNumber.substring(0, 3) === "628") return PhoneNumber;
    }
    phoneNumberInput.value = splitValue.find(checkPhoneNumber); //find return the value that pass a test

    // slice returns selected array elements as a new array
    // join returns array into a string separated by " "
...
function restartNIK() {
  if (alertModalP.style.display === "block") {
    cekNikInput.value = "";
